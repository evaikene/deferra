...
///
/// `Succeeded` and `Cancelled` forbid a failure disposition and retry deadline. `Failed` requires a failure
/// disposition, and permits `retry_not_before` only with `Retryable`. `Interrupted` is reserved for recovery and must
/// not be emitted by normal Phase 4 executors. `result` must be a user-safe JSON object whose deterministic serialized
/// form is at most 256 KiB. `output` is optional and, when present, must satisfy the channel invariants and the
/// immutable run's capture policy. Output bytes remain separate from result metadata and are committed atomically with
/// completion state.
///
...
public:
    ...
    /// failure does not undo durable running state; the scheduler converts the safe returned error into terminal
    /// attempt completion.
    ///
