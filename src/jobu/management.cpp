...
struct ManagementService::Private : jb::core::priv::ObjectPrivate {
...
auto ManagementService::create_queue(CreateQueueRequest request) -> jb::core::Result<Queue, jb::core::Error>
{
    ...
    if (data->initialization_error) {
        return ServiceResult<Queue>::failure(*data->initialization_error);
    }
    ...
    auto defaults = validate_attributes(data->attributes, request.defaults, AttributeScope::QueueDefault);
    if (!defaults) {
    ...
    if (request.idempotency_key) {
        auto encoded = detail::encode_queue_create_idempotency_request(request, data->attributes);
        if (!encoded) {
    ...
    auto id = data->uuid_generator.generate();
    if (!id) {
    ...
    auto inserted = data->queues.insert(queue, queue.name, *defaults);
    if (!inserted) {
        return ServiceResult<Queue>::failure(std::move(inserted).error());
    }
    if (request.idempotency_key) {
        auto result_json = detail::encode_queue_idempotency_result(queue, data->attributes);
        if (!result_json) {
            return ServiceResult<Queue>::failure(std::move(result_json).error());
        }
        auto recorded = data->idempotency.insert({
            .method       = "queue.create",
...
auto ManagementService::list_queues(QueueListRequest const& request) -> jb::core::Result<QueuePage, jb::core::Error>
{
    ...
    if (data->initialization_error) {
        return ServiceResult<QueuePage>::failure(*data->initialization_error);
    }
...
auto ManagementService::update_queue(UpdateQueueRequest request) -> jb::core::Result<Queue, jb::core::Error>
{
    ...
    if (request.defaults) {
        auto validated = validate_attributes(data->attributes, *request.defaults, AttributeScope::QueueDefault);
        if (!validated) {
            return ServiceResult<Queue>::failure(std::move(validated).error());
        }
...
auto ManagementService::suspend_queue(QueueSelector const& selector) -> jb::core::Result<Queue, jb::core::Error>
{
    ...
    if (queue.state == QueueState::Active) {
        ...
        auto transitioned = data->queues.set_state(queue.id, QueueState::Active, QueueState::Suspending, now);
        if (!transitioned) {
    ...
    auto running = data->runs.count_running_for_queue(queue.id);
    if (!running) {
        return ServiceResult<Queue>::failure(std::move(running).error());
    }
    if (*running == 0) {
        auto transitioned = data->queues.set_state(queue.id, QueueState::Suspending, QueueState::Suspended, now);
        if (!transitioned) {
            return ServiceResult<Queue>::failure(std::move(transitioned).error());
        }
        if (!*transitioned) {
...
auto ManagementService::delete_queue(QueueSelector const& selector) -> jb::core::Result<void, jb::core::Error>
{
    ...
    if (data->initialization_error) {
        return ServiceResult<void>::failure(*data->initialization_error);
    }
    auto validated = validate_selector(selector);
    if (!validated) {
    ...
    auto job_count = data->queues.count_non_deleted_jobs(queue.id);
    if (!job_count) {
        return ServiceResult<void>::failure(std::move(job_count).error());
    }
    auto exhausted = data->jobs.has_exhausted_revision_in_queue(queue.id, kMaximumPersistedJobRevision);
    if (!exhausted) {
    ...
    auto deleted_jobs = data->jobs.mark_all_in_queue_deleted(queue.id, now);
    if (!deleted_jobs) {
    ...
    auto cancelled = data->runs.cancel_pending_for_queue(queue.id, now, "queue_deleted");
    if (!cancelled) {
        return ServiceResult<void>::failure(std::move(cancelled).error());
    }

    auto internal_name = queue.name + "-deleted#" + queue.id.to_string();
    auto deleted_queue = data->queues.mark_deleted(queue.id, internal_name, queue.name, now);
    if (!deleted_queue) {
...
auto ManagementService::create_job(CreateJobRequest request) -> jb::core::Result<JobDefinition, jb::core::Error>
{
    ...
    auto partial_attributes = validate_attributes(data->attributes, request.attributes, AttributeScope::Job);
    ...
    auto generate_identity = [&]() -> ServiceResult<void> {
        auto generated_job_id = data->uuid_generator.generate();
        if (!generated_job_id) {
            return ServiceResult<void>::failure(std::move(generated_job_id).error());
        }
        auto generated_run_id = data->uuid_generator.generate();
        if (!generated_run_id) {
    ...
    auto canonical_request = std::optional<std::string>{};
    if (request.idempotency_key) {
        auto encoded = detail::encode_job_create_idempotency_request(request, queue.id, data->attributes);
        if (!encoded) {
    ...
    if (auto const* once = std::get_if<OnceSchedule>(&request.schedule)) {
        ...
        auto next = data->cron.next_after(cron_schedule, now);
        if (!next) {
    ...
    if (request.idempotency_key) {
        auto result_json = detail::encode_job_idempotency_result(job, data->attributes);
        if (!result_json) {
...
auto ManagementService::run_now(RunNowRequest request) -> jb::core::Result<JobRun, jb::core::Error>
{
    ...
    if (data->initialization_error) {
        return ServiceResult<JobRun>::failure(*data->initialization_error);
    }
    auto idempotency = validate_idempotency_key(request.idempotency_key);
    if (!idempotency) {
    ...
    auto busy = data->runs.has_running_or_retrying_run(job.id);
    if (!busy) {
    ...
    auto manual = data->runs.has_non_terminal_manual_run(job.id);
    if (!manual) {
    ...
    auto run_id = data->uuid_generator.generate();
    if (!run_id) {
    ...
    if (request.idempotency_key) {
        auto result_json = detail::encode_run_now_idempotency_result(run, data->attributes);
        if (!result_json) {
...
auto ManagementService::update_job(UpdateJobRequest request) -> jb::core::Result<JobDefinition, jb::core::Error>
{
    ...
    if (!recurring) {
        auto started = data->attempts.has_started_for_job(replacement.id);
        if (!started) {
    ...
    if ((**schedule_owned).state == RunState::Scheduled) {
        auto has_attempt = data->attempts.has_any_for_run((**schedule_owned).id);
        if (!has_attempt) {
    ...
    auto replaced =
        data->jobs.update_definition(replacement, request.expected_revision, *serialized_attributes, *payload);
    if (!replaced) {
    ...
    if (refresh_snapshot) {
        ...
        if (auto const* once = std::get_if<OnceSchedule>(&replacement.schedule)) {
            ...
            auto next = data->cron.next_after(std::get<CronSchedule>(replacement.schedule), now);
            if (!next) {
                return ServiceResult<JobDefinition>::failure(std::move(next).error());
            }
            planned_at = *next;
        ...
        auto refreshed =
            data->runs.refresh_unstarted_schedule_owned(replacement.id,
                                                        {
...
auto ManagementService::suspend_job(jb::core::Uuid const& id) -> jb::core::Result<JobDefinition, jb::core::Error>
{
    ...
    if (job.state == JobState::Active) {
        ...
        auto       transitioned =
            data->jobs.set_state(job.id, JobState::Active, JobState::Suspending, expected_revision, next_revision, now);
        if (!transitioned) {
    ...
    if (*running == 0) {
        ...
        auto const expected_revision = job.revision;
        auto const next_revision     = expected_revision + 1;
        auto       transitioned      = data->jobs.set_state(job.id,
                                                            JobState::Suspending,
                                                            JobState::Suspended,
                                                            expected_revision,
                                                            next_revision,
...
auto ManagementService::resume_job(jb::core::Uuid const& id) -> jb::core::Result<JobDefinition, jb::core::Error>
{
    ...
    if (job.state != JobState::Active) {
        ...
        auto const next_revision     = expected_revision + 1;
        auto       transitioned =
            data->jobs.set_state(job.id, expected_state, JobState::Active, expected_revision, next_revision, now);
        if (!transitioned) {
            return ServiceResult<JobDefinition>::failure(std::move(transitioned).error());
        }
        if (!*transitioned) {
...
auto ManagementService::move_job(MoveJobRequest const& request) -> jb::core::Result<JobDefinition, jb::core::Error>
{
    ...
    auto       moved_job     = data->jobs.move(job.id, job.revision, target_queue.id, next_revision, now);
    if (!moved_job) {
...
auto ManagementService::delete_job(DeleteJobRequest const& request) -> jb::core::Result<void, jb::core::Error>
{
    ...
    auto references = data->secrets.replace_references_for_job(job.id, {});
    if (!references) {
        return ServiceResult<void>::failure(std::move(references).error());
    }
    auto committed = transaction.commit();
...
auto ManagementService::list_jobs(JobListRequest const& request) -> jb::core::Result<JobPage, jb::core::Error>
{
    ...
    if (data->initialization_error) {
        return ServiceResult<JobPage>::failure(*data->initialization_error);
    }
    if (request.page.limit == 0 || request.page.limit > kMaximumPageSize) {
    ...
    auto listed = data->jobs.list(queue_id,
