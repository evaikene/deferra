...
///
/// Call on the open, idle database's owner thread while holding exclusive daemon database
/// ownership. Dependencies are borrowed only for this call. No executor is invoked, and no
/// listener, event loop, or scheduler is started. Recovery uses current queue policy and
...
/// and reopen after storage failure and rerun recovery before serving. Repeated recovery is
/// safe and does not duplicate retries or recurring successors. Success proves no Running
/// work remains, required recurring work exists, barriers are valid, and suspensions drained.
///
