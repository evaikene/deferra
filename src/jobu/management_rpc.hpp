...
/// borrow @p service and @p attributes, which must outlive the registrations created by this call. @p attributes must
/// be the registry used to construct @p service so request and response conversion share the same attribute contract.
///
/// A successful mutating service call emits ManagementService::mutation_committed before the adapter begins response
/// encoding. The adapter does not own a second mutation-notification mechanism.
