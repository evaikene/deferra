...
auto capture_error(std::string detail) -> jb::core::Error
{
    return {
        ...
        .code     = "jobu.cli.capture_failed",
        .message  = "CLI output capture failed",
...
auto checked_cli_capture_total(std::uint64_t current_total, std::size_t appended_bytes)
    -> jb::core::Result<std::uint64_t, jb::core::Error>
{
    using Result = jb::core::Result<std::uint64_t, jb::core::Error>;

    if constexpr (std::numeric_limits<std::size_t>::digits > std::numeric_limits<std::uint64_t>::digits) {
        if (appended_bytes > static_cast<std::size_t>(std::numeric_limits<std::uint64_t>::max())) {
            return Result::failure(capture_error("capture_total_overflow"));
        }
    }
    auto const appended = static_cast<std::uint64_t>(appended_bytes);
    if (appended > std::numeric_limits<std::uint64_t>::max() - current_total) {
        return Result::failure(capture_error("capture_total_overflow"));
    }
    return Result::success(current_total + appended);
}

CliCaptureBuffer::CliCaptureBuffer(std::size_t limit)
    : _limit{limit}
{}

auto CliCaptureBuffer::append(jb::core::ByteView bytes) -> jb::core::Result<void, jb::core::Error>
{
    using Result = jb::core::Result<void, jb::core::Error>;

    auto total = checked_cli_capture_total(_total_bytes, bytes.size());
    if (!total) {
        return Result::failure(std::move(total).error());
    }

    // Retention stays bounded after truncation, while total byte accounting continues so output limits never become
    // process backpressure and result metadata still describes the complete observed stream.
...
void CliCaptureBuffer::append_suffix(jb::core::ByteView bytes)
{
    ...
    // Once full, the suffix is a ring. _suffix_start identifies the oldest byte so take() can restore stream order.
...
auto CliCaptureBuffer::take() -> AttemptOutputChannel
{
    ...
    auto       result        = AttemptOutputChannel{
