...
void LocalSocket::Private::complete_connection(LocalSocket& socket)
{
    ...
    uid_t user_id{};
    gid_t group_id{};
    if (!query_peer_credentials(active_fd, user_id, group_id)) {
        auto const native_error = errno;
        fail_lifecycle(socket,
                       open_error(native_error),
    ...
    peer_credentials.process_id.reset();
    peer_credentials.user_id  = static_cast<std::uint64_t>(user_id);
    peer_credentials.group_id = static_cast<std::uint64_t>(group_id);
...
void LocalSocket::Private::write_pending(LocalSocket& socket)
{
    ...
    while (!output_buffer.empty()) {
        auto const n = ::send(active_fd, output_buffer.data(), output_buffer.size(), 0);
        if (n > 0) {
