...
/// limits. A framing failure clears retained bytes and poisons the framer: later append operations return the same
/// error until reset() is called for deliberate reuse after abandoning the failed connection. Returned bodies own their
/// bytes and exclude framing headers; no body is interpreted as UTF-8 or JSON.
///
