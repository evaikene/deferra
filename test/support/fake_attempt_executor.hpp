...
/// `test.executor.type_unavailable` without retaining its handler. Starts and cancellations run only on the calling
/// test thread, no thread or external operation is created, and successful starts remain pending until complete() is
/// called.
///
