...
struct FaultOpenDatabase {
    ...
    FaultOpenDatabase() { REQUIRE(database.open()); }
};
...
TEST_CASE("JobU SQLite schema permits terminal states only for one-time jobs", "[jobu][sqlite][schema]")
{
    OpenDatabase fixture;
    REQUIRE(jobu_sqlite::ensure_schema(fixture.database));
    insert_queue(fixture.database);
    ...
    for (auto const* state : {"active", "suspending", "suspended"}) {
        execute(fixture.database, std::string{"UPDATE jobu_jobs SET state = '"} + state + "' WHERE name = 'example'");
    }
    for (auto const* state : {"succeeded", "failed", "cancelled"}) {
        execute(fixture.database, std::string{"UPDATE jobu_jobs SET state = '"} + state + "' WHERE name = 'example'");
        CHECK(execute_error(fixture.database, "UPDATE jobu_jobs SET deleted_at_us = 10 WHERE name = 'example'").code ==
              "db.constraint");
    }
    CHECK(execute_error(fixture.database, "UPDATE jobu_jobs SET state = 'unknown' WHERE name = 'example'").code ==
          "db.constraint");

    execute(fixture.database, "UPDATE jobu_jobs SET state = 'deleted', deleted_at_us = 10 WHERE name = 'example'");
    CHECK(execute_error(fixture.database, "UPDATE jobu_jobs SET state = 'active' WHERE name = 'example'").code ==
          "db.constraint");
    execute(fixture.database, "UPDATE jobu_jobs SET state = 'active', deleted_at_us = NULL WHERE name = 'example'");

    execute(fixture.database,
            "UPDATE jobu_jobs SET schedule_kind = 'cron', scheduled_at_us = NULL, "
            "cron_expression = '* * * * *', cron_timezone = 'UTC' WHERE name = 'example'");
    for (auto const* state : {"succeeded", "failed", "cancelled"}) {
        CHECK(execute_error(fixture.database,
                            std::string{"UPDATE jobu_jobs SET state = '"} + state + "' WHERE name = 'example'")
                  .code == "db.constraint");
    }
    REQUIRE(jobu_sqlite::ensure_schema(fixture.database));
}
...
TEST_CASE("JobU SQLite schema rejects older markers without changing data", "[jobu][sqlite][schema]")
{
    ...
    execute(fixture.database, "UPDATE jobu_schema SET version = " + std::to_string(version));
    auto error = require_schema_error(jobu_sqlite::ensure_schema(fixture.database),
                                      ErrorCategory::Unsupported,
                                      "jobu.schema.unsupported_version");
    CHECK(error.message.find("fresh database") != std::string::npos);
    CHECK(scalar_integer(fixture.database, "SELECT version FROM jobu_schema") == version);
...
TEST_CASE("JobU SQLite schema rejects a current marker over incompatible job constraints", "[jobu][sqlite][schema]")
{
    OpenDatabase fixture;

    // Build one controlled incompatible table with the current columns, indexes, and marker.
    for (auto const& object : jobu_sqlite::detail::schema_object_manifest()) {
        ...
        if (object.name == "jobu_jobs") {
            auto const states = std::string{"'succeeded', 'failed', 'cancelled'"};
            auto const offset = definition.find(states);
            REQUIRE(offset != std::string::npos);
            definition.replace(offset, states.size(), "'succeeded', 'failed', 'retired'");
        }
...
TEST_CASE("JobU SQLite schema rejects incompatible history index shapes", "[jobu][sqlite][schema]")
{
...
TEST_CASE("JobU SQLite creation rolls back failed table validation", "[jobu][sqlite][schema][fault]")
{
    ...
    fixture.faults->faults.push_back({
        .at    = {.boundary = "jobs.definition", .operation = DatabaseOperation::Execute},
        .error = fault_error()
    });
...
TEST_CASE("JobU SQLite current-format validation does not repair or mutate on read failure",
          "[jobu][sqlite][schema][fault]")
{
    FaultOpenDatabase fixture;
    REQUIRE(jobu_sqlite::ensure_schema(fixture.database));
    insert_queue(fixture.database);
