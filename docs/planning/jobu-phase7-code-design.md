...
Use a daemon-owned POSIX self-pipe relay, with nonblocking close-on-exec descriptors and an event-loop watch. The handler only preserves/restores `errno`, sets a monotonic `volatile sig_atomic_t` shutdown flag, and performs a best-effort async-signal-safe write. It performs no allocation, logging, JSON, Object access, or teardown. Full-pipe writes may be dropped because the flag and existing readability preserve the request. The flag is never cleared during this daemon lifetime.
...
Keep the relay alive until objects that may create threads are destroyed. Restore dispositions and retire descriptors only when no handler can access a closed/reused descriptor; blocking only the teardown thread is insufficient if another eligible thread can run the handler. Implement startup before worker creation, mask inheritance deliberately, and teardown after workers join. Document that this is process-main infrastructure, not an embeddable global handler API.
