...
| `jobu.secret.not_found` | NotFound | The secret to delete does not exist. The private execution lookup uses the same code for a missing row. |
| `jobu.secret.in_use` | Conflict | A current-definition reference prevents deletion. There is no force option. |
...
Current-definition references are checked in the delete transaction. Automatic
reference maintenance and deletion protection for older nonterminal snapshots
are the next implementation stage; the current service does not yet provide
that complete deletion protection. There is no public secret-value read API.
